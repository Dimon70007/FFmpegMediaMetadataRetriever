...
then
    ...
    echo "                <ANDROID_TARGET_ABI> <GCC_VERSION> <OUTPUT_PATH> <LIBSSL_PREFIX>"
    echo
    ...
    echo "                /home/user/target_dir/armeabi-v7a"
    echo "expected 7 perameters, but found \\"
    echo "\$1 - $1 \ "
    echo "\$2 - $2 \ "
    echo "\$3 - $3 \ "
    echo "\$4 - $4 \ "
...
openssl_config="shared -no-ssl2 -no-ssl3 -no-hw zlib-dynamic"
...
# openssl_so_flags="$FF_MAKE_FLAGS SHLIB_EXT='.so'"

export CALC_VERSIONS="SHLIB_COMPAT=; SHLIB_SOVER="
function build_library {
    ...
    pushd $OPENSSL_TMP_FOLDER > /dev/null
    ...
    make depend && make && make $FF_MAKE_FLAGS install_sw || exit 1
    rm -rf ${OPENSSL_TMP_FOLDER}
